...
class ModelSTAGIN(nn.Module):
    ...
    def forward(self, v, a, t, sampling_endpoints):
        ...
        for layer, (G, R, T, L) in enumerate(
            ...
            latent = self.cls_token(h_attend)  # latent.shape: (batch_size, 64)
            logit += self.dropout(L(latent))  # logit.shape: (batch_size, 2)
        ...
        latent = torch.stack(latent_list, dim=1)  # (batch_size, 2, 64)
