...
def visualize_sagittal(roi_dict, savedir, min=VMIN, max=VMAX, colorname='4hot'):
    for hemisphere in ['LH', 'RH']:
        for network in NETWORK_LIST:
            initialize()
            if hemisphere == 'LH':
                gl.clipazimuthelevation(0.49, 90, 0)
            elif hemisphere == 'RH':
                gl.clipazimuthelevation(0.49, 270, 0)
            else:
                raise
...
def plot_figure(sourcedir, targetdir=None):
    ...
    network_figures = {}
    for network in NETWORK_LIST:
        axial = cv2.resize(cv2.imread(os.path.join(sourcedir, f'{network}_axial.png'))[150:-150, 300:-300],
                           dsize=(0, 0), fx=1.6, fy=1.6)
        ...
        network_figures[network] = np.concatenate(
            [np.zeros_like(network_figure)[:, :50], network_figure, np.zeros_like(network_figure)[:, :50]], axis=1)
    full_figure = np.concatenate([np.concatenate(
        [network_figures['Default'], network_figures['SalVentAttn'], network_figures['Cont'],
         network_figures['DorsAttn']], axis=1), np.concatenate(
        [network_figures['Limbic'], network_figures['SomMot'], network_figures['Vis'],
         np.zeros_like(network_figures['Vis'])], axis=1)])
    ...
    if TASK:
        for layer in os.listdir(os.path.join(EXP_DIR, 'analysis', 'node_attention', TASK, 'nifti')):
            for contrast in os.listdir(os.path.join(EXP_DIR, 'analysis', 'node_attention', TASK, 'nifti', layer)):
                os.makedirs(os.path.join(EXP_DIR, 'analysis', 'node_attention', TASK, 'image', layer, contrast),
                            exist_ok=True)
                os.makedirs(os.path.join(EXP_DIR, 'analysis', 'figures', TASK, layer, contrast), exist_ok=True)
                roi_dict_node_attention = sort_rois(
                    os.path.join(EXP_DIR, 'analysis', 'node_attention', TASK, 'nifti', layer, contrast), ROIMETA_DIR)

                visualize_axial(roi_dict_node_attention,
                                os.path.join(EXP_DIR, 'analysis', 'node_attention', TASK, 'image', layer, contrast))
                visualize_sagittal(roi_dict_node_attention,
                                   os.path.join(EXP_DIR, 'analysis', 'node_attention', TASK, 'image', layer, contrast))
                plot_figure(os.path.join(EXP_DIR, 'analysis', 'node_attention', TASK, 'image', layer, contrast),
                            os.path.join(EXP_DIR, 'analysis', 'figures', TASK, layer, contrast))
        ...
        for layer in os.listdir(os.path.join(EXP_DIR, 'analysis', 'node_attention', 'nifti')):
            ...
            roi_dict_node_attention = sort_rois(os.path.join(EXP_DIR, 'analysis', 'node_attention', 'nifti', layer),
                                                ROIMETA_DIR)

            visualize_axial(roi_dict_node_attention,
                            os.path.join(EXP_DIR, 'analysis', 'node_attention', 'image', layer))
            visualize_sagittal(roi_dict_node_attention,
                               os.path.join(EXP_DIR, 'analysis', 'node_attention', 'image', layer))
            plot_figure(os.path.join(EXP_DIR, 'analysis', 'node_attention', 'image', layer),
                        os.path.join(EXP_DIR, 'analysis', 'figures', layer))
