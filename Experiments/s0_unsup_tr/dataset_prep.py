...
class Combine_Dataset(Dataset):
    def __init__(self, data_dir):
        ...
        with open(data_dir + "combine_ADHD_ABIDE_MDD_3806subj_TP170_data.pkl", "rb") as fp:
            series = pickle.load(fp)  # 'series' is a list, with each element size: T*N

        sample = len(series)
        numbers = [int(x) for x in range(sample)]  # 'numbers' matches combine_ADHD_ABIDE_MDD_3720subj_TP170_fileID.npy
    ...
    def __getitem__(self, idx):
        ...
        return {'id': subject, 'timeseries': tensor(timeseries, dtype=float32)}
